...
func ndecided(t *testing.T, pxa []*Paxos, seq int) int {
	...
	for i := 0; i < len(pxa); i++ {
		if pxa[i] != nil {
			...
			//log.Println("The decided value: ", v1)
			if decided {
				if count > 0 && v != v1 {
...
func TestForgetMem(t *testing.T) {
	...
	log.Println("How Many Mem0 using: ", m0.Alloc)
	...
	log.Println("How Many Mem1 using: ", m1.Alloc)
	...
	log.Println("How Many Mem2 using: ", m2.Alloc)
