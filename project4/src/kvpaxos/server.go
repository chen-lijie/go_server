...
func (kv *KVPaxos) WaitAllDone(startnum int, seqnum int) (string, bool) {
	...
	for i := startnum; i <= seqnum; i++ {
		...
		if decided.Type == GET {
			...
			success = true
		} else if decided.Type == PUT {
			pv, exist := kv.database[decided.Key]
			success = true
			if exist {
				result = pv
			} else {
...
func (kv *KVPaxos) GetRPC(args *GetArgs, reply *GetReply) error {
	key, uuid, _ := args.Key, args.UUID, args.Me
	result, _ := kv.Get(key, strconv.FormatInt(uuid,10))
	reply.Value = result
	return nil
}


func (kv *KVPaxos) PutRPC(args *PutArgs, reply *PutReply) error {
	key, value, dohash, uuid, _ := args.Key, args.Value, args.DoHash, args.UUID, args.Me
	result,_ := kv.Put(key, value, strconv.FormatInt(uuid,10))
	if dohash {
...
func (kv *KVPaxos) Put(key string, value string, session_id string) (string,bool) {
	...
	op := Op{PUT, key, value, session_id}
	seqnum := kv.seq
	startnum := seqnum
	for {
