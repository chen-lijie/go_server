...
func (kv *KVPaxos) PutRPC(args *PutArgs, reply *PutReply) error {
	...
	result,_ := kv.Put(key, value, ck, uuid)
	if dohash {
...
func (kv *KVPaxos) Put(key string, value string, id string, seq int64) (string,bool) {
	kv.mu.Lock()
	defer kv.mu.Unlock()

	op := Op{id, seq, PUT, key, value}
	seqnum := kv.seq
	startnum := seqnum
	for {
